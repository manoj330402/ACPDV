...
uses
  ...
  Vcl.Mask, FireDAC.Phys.SQLite, System.SysUtils;
...
//    FConnection.Params.Add('Server=localhost');
    ...
    with FQuery do
    begin
      SQL.Text := 'SELECT * FROM sqlite_master WHERE type=''table'' order by name;';
      Open;
      First;
      while not Eof do
      begin
        chkListaTabelas.Items.Add(EliminaBrancos(FieldByName('name').AsString));
        Next();
      end;
    end;
...
var tabela, campo, diretorio_banco : string;
    i, j : Integer;
begin
  try
  caminho_banco := edtCaminhoArquivos.Text;
  if not FileExists(caminho_banco) then
  begin
    ShowMessage('Base de dados não encontrada');
    Abort;
...
//      script.Lines.Add('  [Tabela('+QuotedStr(maiusculas_sem_acento_e_cedilha(tabela))+')]');
    ...
    //FQuery.Fields[i].ClassName+' - '+FQuery.Fields[i].Size.ToString
      ...
      diretorio_banco := StringReplace(edtCaminhoArquivos.Text, ExtractFileName(edtCaminhoArquivos.Text), '', [rfReplaceAll, rfIgnoreCase]);

      if not DirectoryExists(diretorio_banco) then
        CreateDir(diretorio_banco);
      script.Lines.SaveToFile(diretorio_banco + edtPrefixoEntidades.Text + '.' + maiusculas_sem_acento_e_cedilha(tabela) + '.pas');
...
begin
  ...
  if not DirectoryExists(diretorio_banco) then
    CreateDir(diretorio_banco);
  script.Lines.SaveToFile(diretorio_banco + edtPrefixoModel.Text + '.Model.' + maiusculas_sem_acento_e_cedilha(tabela) + '.Interfaces.pas');
  ...
  if not DirectoryExists(diretorio_banco) then
    CreateDir(diretorio_banco);
  script.Lines.SaveToFile(diretorio_banco + edtPrefixoModel.Text + '.Model.' + maiusculas_sem_acento_e_cedilha(tabela) + '.pas');
  ...
  if not DirectoryExists(diretorio_banco) then
    CreateDir(diretorio_banco);
  script.Lines.SaveToFile(diretorio_banco + edtPrefixoModel.Text + '.Controller.' + maiusculas_sem_acento_e_cedilha(tabela) + '.Interfaces.pas');
  ...
  if not DirectoryExists(diretorio_banco) then
    CreateDir(diretorio_banco);
  script.Lines.SaveToFile(diretorio_banco + edtPrefixoModel.Text + '.Controller.' + maiusculas_sem_acento_e_cedilha(tabela) + '.pas');
  script.Lines.Clear;
...
begin
  try
    caminho_banco := 'C:\Users\Jairo-F\Documents\Embarcadero\Studio\Projects\ACPDV\db\ACPDV_BD.db';
    if not FileExists(caminho_banco) then
    begin
      ShowMessage('Base de dados não encontrada');
      Abort;
...
begin
  ...
  Result := StringReplace(AnsiLowerCase(aux), aux[1], AnsiUpperCase(aux[1]), [rfReplaceAll, rfIgnoreCase]);
end;
