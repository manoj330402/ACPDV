...
type
  ...
  public
    ...
    function FindWhere(aField: string; aValue: Variant): TDAOCaixa;
  end;
...
function TDAOCaixa.FindWhere(aField: string; aValue: Variant): TDAOCaixa;
begin
  Result := Self;

  FDataSet := FConexaoDB.SQL(
    'SELECT * FROM CAIXA WHERE ' + aField + ' = ?'
  )
  .Params(0, aValue)
  .Open
  .DataSet;
